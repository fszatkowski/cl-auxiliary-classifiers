...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        ...
        for i, exp_name in enumerate(exp_names):
            ...
            means = means[:, 1:]
            heatmap = sns.heatmap(
                means, vmin=0, vmax=1, annot=True, cbar=False, fmt=".2f", ax=axes[i]
            )
            ...
            axes[i].set_xticklabels([i + 1 for i in list(range(means.shape[1]))])
        ...
        plt.tight_layout()
        plt.savefig(str(output_dir / f"{dataset}.png"))
        plt.savefig(str(output_dir / f"{dataset}.pdf"))
