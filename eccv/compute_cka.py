    ...
    with torch.no_grad():
        for feature_dir in tqdm(feature_dirs):
            ...
            for task_dir in task_dirs:
                ...
                task_ids = sorted(
                    list(
                        set([task_file.name.split("_")[1] for task_file in task_files])
                    )
                )
                ...
                for id in task_ids:
                    batch_files = [
                        ...
                        if f"task_{id}" in task_file.name
                    ]
                    # load batches, list of tensors for each intermediate layer
                    batches = [
                        torch.load(batch_file, map_location="cpu")
            ...
            for task_id in range(n_tasks):
                task_data = [
                    task_to_features[i][task_id] for i in range(task_id, n_tasks)
                ]
                ...
                for feat_idx in range(7):
                    for j in range(len(task_data) - 1):
                        ...
                        feats_cur_task = feats_cur_task.view(
                            feats_cur_task.shape[0], -1
                        )
                        feats_next_task = feats_next_task.view(
                            feats_next_task.shape[0], -1
                        )
                        ckas[feat_idx, task_id, j + 1] = _CKA(
                            feats_cur_task.to(device), feats_next_task.to(device)
                        ).item()
