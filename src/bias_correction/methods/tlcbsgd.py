...
class TLCBSGD(BiasCorrection):
    ...
    def fit_bias_correction(
        ...
        a = torch.nn.Parameter(torch.zeros((1, 1, 1), device=self.device))
        b = torch.nn.Parameter(torch.zeros((1, 1, 1), device=self.device))
        optimizer = torch.optim.AdamW([a, b], lr=self.lr)
        pbar = tqdm(
            range(self.n_steps), desc="Optimizing TLCB bias correction through SGD..."
        )

        arange = torch.arange(9, -1, -1).unsqueeze(0).unsqueeze(0).to(self.device)
        bias_mask = torch.ones((1, 1, 10), device=self.device)
        bias_mask[0, 0, -1] = 0

        for _ in pbar:
            optimizer.zero_grad()
            taskwise_bias = arange * a + b.repeat_interleave(10, dim=2) * bias_mask
        ...
        self._bias = (
            (a * arange + (b.repeat_interleave(10, dim=2) * bias_mask).squeeze())
            .detach()
            .to("cpu")
        )
