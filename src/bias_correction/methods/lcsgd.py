...
class LCSGD(BiasCorrection):
    ...
    def fit_bias_correction(
        ...
        bias = torch.nn.Parameter(torch.zeros((1, 1, self.n_tasks), device=self.device))
        optimizer = torch.optim.AdamW([bias], lr=self.lr)
        pbar = tqdm(
            range(self.n_steps), desc="Optimizing LC bias correction through SGD..."
        )
