...
class MeanLCTSGD(BiasCorrection):
    def __init__(
        self,
        n_tasks: int,
        n_cls: int,
    ...
    def fit_bias_correction(
        ...
        logits_train = train_data.logits.to(self.device)
        ...
        preds_masks[preds_masks == 0] = 0
        per_task_logits_train = logits_train * preds_masks
        mean_ood_logits = per_task_logits_train.sum(dim=-1) / preds_masks.sum(dim=-1)
        ...
        for _ in pbar:
            optimizer.zero_grad()
            adapted_logits = logits_train + bias.repeat_interleave(
                self.classes_per_task, dim=2
            )
            loss = adapted_logits - mean_ood_logits.unsqueeze(-1)
            loss = (loss**2).mean()
            loss.backward()
            optimizer.step()
