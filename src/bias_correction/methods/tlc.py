...
class TLC(BiasCorrection):
    ...
    def fit_bias_correction(
        ...
        logits = train_data.logits.to(self.device)
        ...
        def loss_fn(bias_dict):
            ...
            per_task_preds = torch.stack(
                torch.split(masked_logits, self.classes_per_task, dim=2), dim=-1
        ...
        self._expanded_bias = (
            torch.repeat_interleave(best_bias, self.classes_per_task, dim=1)
            .unsqueeze(0)
            .to("cpu")
        )
...
def expand_bias(bias: Tensor, classes_per_task: int) -> Tensor:
    return torch.repeat_interleave(bias, classes_per_task, dim=1)
