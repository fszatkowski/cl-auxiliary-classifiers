...
class HyperoptTemperatureCorrection(BiasCorrection):
    def __init__(
        self,
        n_tasks: int,
        n_cls: int,
        ...
        if hp_space == "normal":
            hp_init = {
                f"t_{ic_idx}_{task_idx}": hp.normal(
                    f"t_{ic_idx}_{task_idx}", mu=hp_mu, sigma=hp_sigma
                )
            ...
            hp_init = {
                f"t_{ic_idx}_{task_idx}": hp.uniform(
                    f"t_{ic_idx}_{task_idx}", low=hp_min, high=hp_max
                )
                for ic_idx in range(self.n_cls)
                for task_idx in range(self.n_tasks)
            }
    ...
    def fit_bias_correction(
        ...
        def loss_fn(bias_dict):
            bias = decode_bias_dict(bias_dict, self.n_cls, self.n_tasks, self.device)
            bias = (
                bias.unsqueeze(0)
            ...
            temperatures = (pred_mask * bias).sum(dim=-1).unsqueeze(-1)
        ...
        best_bias = decode_bias_dict(
            best_coefficients, self.n_cls, self.n_tasks, self.device
        )

        self.exit_costs = self.exit_costs.to("cpu")
        self.thresholds = self.thresholds.to("cpu")
        self._bias = best_bias.to("cpu")
        self._extended_bias = (
            self._bias.unsqueeze(0)
            .repeat_interleave(self.classes_per_task, dim=2)
            .to("cpu")
        )
        self._losses = trials.losses()
...
def decode_bias_dict(bias_dict: Dict, n_ics: int, n_tasks: int, device: str) -> Tensor:
    bias = torch.zeros(n_ics, n_tasks)

    for ic_idx in range(n_ics):
        for task_idx in range(n_tasks):
            bias[ic_idx, task_idx] = bias_dict[f"t_{ic_idx}_{task_idx}"]
