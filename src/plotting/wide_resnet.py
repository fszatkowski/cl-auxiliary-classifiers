...
PLOT_SETTINGS = [["CIFAR100x5", "CIFAR100x10"]]
PLOT_TITLES = [
    ["CIFAR100x5 | WideResNet16", "CIFAR100x10 | WideResNet16"],
]
...
if __name__ == "__main__":
    ...
    averaged_scores = load_averaged_scores(
        ROOT_DIR / "results_wideresnet", downsample=DOWNSAMPLE, filter=filter_fn
    )
    averaged_scores = sorted(
        averaged_scores, key=lambda x: (x.metadata.setting, x.metadata.exp_name)
    )
    ...
    def setup_fn(x):
        if isinstance(x, str):
            ...
            if "sdn" in x["exp_name"]:
                setup = "+AC"
            else:
                setup = "Base"
        else:
            raise ValueError('Invalid setup type: "{}"'.format(type(x)))
        return setup
        ...
        for combined_plot_settings, combined_plot_titles in zip(
            ...
            for ax_idx, (setting, title) in enumerate(
                ...
                if ax_idx == len(combined_plot_settings) - 1:
                    ...
                    labels = [l if not "+AC" in l else l.split(" ")[1] for l in labels]
                    # Make handles shorter

                    if len(handles) == 10:
                        legend_fontsize = FONTSIZE_LEGEND - 2
                    elif len(handles) == 12:
                        legend_fontsize = FONTSIZE_LEGEND - 4
                    else:
                        raise ValueError()
                    plot.legend(
                        handles=handles,
            ...
            plt.tight_layout()
            fig.savefig(OUTPUT_DIR_PLOTS / f"wideresnet_{batch_idx}.pdf")

    # ------------------------------------

    # TABLE
    OUTPUT_DIR_TABLES.mkdir(parents=True, exist_ok=True)
    table_data = load_scores_for_table(
        ROOT_DIR / "results_wideresnet", filter=filter_fn
    )
    ...
    for setting in SETTINGS:
        ...
        df = df.sort_values(
            by=["Setup"],
            key=lambda x: x.map({"Base": 0, "+AC": 1}),
            ascending=True,
        )
        df.to_csv(OUTPUT_DIR_TABLES / f"wideresnet_{setting}.csv", index=False)
        dfs_combined.append(df)

    combined_df = pd.concat(dfs_combined, axis=0)
    combined_df.to_csv(OUTPUT_DIR_TABLES / "combined_wideresnet.csv", index=False)

    md_string = combined_df.to_markdown(index=False)
    md_string = md_string.replace("\\tiny{", "").replace("}", "")
    md_output_path = OUTPUT_DIR_TABLES / "combined_wideresnet.md"
