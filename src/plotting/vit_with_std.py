...
if __name__ == "__main__":
    ...
    for method in sorted(METHODS.keys()):
        method_scores = [
            s for s in averaged_scores_vit if s.metadata.exp_name.startswith(method)
        ]
    ...
    for batch_idx, method_batch in enumerate(PLOT_METHODS):
        for combined_plot_settings, combined_plot_titles in zip(
            ...
            for ax_idx, (setting, title) in enumerate(
                ...
                setting_scores = [
                    s for s in averaged_scores_vit if s.metadata.setting == setting
                ]
                method_to_scores = {}
            ...
            plot.get_figure().savefig(
                OUTPUT_DIR_PLOTS / f"vit_{n_tasks}_{batch_idx}_std.pdf"
            )
            plot.get_figure().savefig(
                OUTPUT_DIR_PLOTS / f"vit_{n_tasks}_{batch_idx}_std.png"
            )
