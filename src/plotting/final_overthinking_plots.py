...
def plot_overthinking(
        data: np.ndarray, ykey: str, output_path: Path, title: str = None, plot_avg: bool = True,
...
def get_overthinking_data(path: Path, device: str):
    ...
    for task_id in range(len(task_dirs)):
        ...
        for batch in batches_paths:
            ...
            hits_tag_any += ((preds_tag == targets_tag.unsqueeze(1)).sum(dim=-1) > 0).sum().item()
            hits_taw_final += (preds_taw[:, -1] == targets_taw).sum().item()
            hits_taw_any += ((preds_taw == targets_taw.unsqueeze(1)).sum(dim=-1) > 0).sum().item()
    ...
    wavg_taw_acc_final = sum([o['taw_acc_final'] * o['total'] for o in outputs]) / total_samples
    wavg_taw_acc_any = sum([o['taw_acc_any'] * o['total'] for o in outputs]) / total_samples
    wavg_tag_acc_final = sum([o['tag_acc_final'] * o['total'] for o in outputs]) / total_samples
    wavg_tag_acc_any = sum([o['tag_acc_any'] * o['total'] for o in outputs]) / total_samples
...
def plot_overthinking_between_tasks(result_paths: List[Path], output_dir: Path, device: str):
    ...
    iterator = tqdm(list(itertools.product(settings, methods, ee_configs)), desc="Plotting...")

    output_dir.mkdir(exist_ok=True, parents=True)
    for (setting, method, ee_config) in iterator:
        tmp_df = df[(df['setting'] == setting) & (df['method'] == method) & (df['ee_config'] == ee_config)]
        if len(tmp_df) == 0:
            continue
...
if __name__ == "__main__":
    ...
    output_dir = root / "analysis_outputs" / "final_overthinking"
