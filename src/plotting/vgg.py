...
PLOT_TITLES = [
    ["CIFAR100x5 | VGG19", "CIFAR100x10 | VGG19"],
]
...
if __name__ == "__main__":
    ...
    def setup_fn(x):
        if isinstance(x, str):
            ...
            if "dense" in x["exp_name"]:
                setup = "+18AC"
            elif "medium" in x["exp_name"]:
                setup = "+10AC"
            elif "small" in x["exp_name"]:
                setup = "+6AC"
            else:
                setup = "Base"
        else:
    ...
    for batch_idx, method_batch in enumerate(PLOT_METHODS):
        for combined_plot_settings, combined_plot_titles in zip(
            ...
            fig, axes = plt.subplots(
                1,
                len(combined_plot_settings),
                figsize=FIGSIZE_APPENDIX_SINGLE_METHOD_ABLATIONS,
            )
            ...
            plt.tight_layout()
            fig.savefig(OUTPUT_DIR_PLOTS / f"vgg19_{batch_idx}.pdf")
    ...
    for setting in SETTINGS:
        ...
        df = df.sort_values(
            by=["Setup"],
            key=lambda x: x.map({"Base": 0, "+6AC": 1, "+10AC": 2, "+18AC": 3}),
            ascending=True,
        )
        df.to_csv(OUTPUT_DIR_TABLES / f"vgg19_{setting}.csv", index=False)
        dfs_combined.append(df)
    combined_df = pd.concat(dfs_combined, axis=0)
    combined_df.to_csv(OUTPUT_DIR_TABLES / "combined_vgg19.csv", index=False)
