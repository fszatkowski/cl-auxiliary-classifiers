...
SETTINGS = ["CIFAR100x5", "CIFAR100x10", "ImageNet100x5_rn18", "ImageNet100x10_rn18"]


PLOT_SETTINGS = [
    ["CIFAR100x5", "ImageNet100x5_rn18"],
    ["CIFAR100x10", "ImageNet100x10_rn18"],
]
PLOT_TITLES = [
    ["CIFAR100x5 | ResNet32", "ImageNet100x5 | ResNet18"],
    ["CIFAR100x10 | ResNet32", "ImageNet100x10 | ResNet18"],
]
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR_PLOTS = ROOT_DIR / "icml_data" / " teaser"
    ...
    for batch_idx, method_batch in enumerate(PLOT_METHODS):
        for combined_plot_settings, combined_plot_titles in zip(
            ...
            plt.clf()
            plt.close("all")
            fig, axes = plt.subplots(len(combined_plot_settings), 1, figsize=(8, 8))
            ...
            for ax_idx, (setting, title) in enumerate(
                ...
                for method in df_combined["method"].unique():
                    ...
                    base_acc = df_method[df_method["setup"] == "Base"]["acc"].values[0]
                    df_ac = df_method[df_method["setup"] == "+AC"]
                    df_ac = df_ac[df_ac["acc"] >= base_acc]
                    min_cost = df_ac["cost"].min()
                    # Plot a marker at the minimum cost and base acc
                    # Point an arrow from the right-top to this marker
                    # Say "<min_cost> % computation
                    plot.scatter(
                        min_cost,
                        base_acc,
                        color="black",
                    ...
                    plot.annotate(
                        f"{min_cost:.0f}%",
                        xy=(min_cost, base_acc),
                        textcoords="offset points",
                        xytext=(0, 5),
                        ha="center",
                        va="bottom",
                        fontsize=FONTSIZE_LABELS_SC - 6,
                        # arrowprops=dict(
                        #     arrowstyle="->",
                        #     color="black",
                    ...
                    #     "",
                    #     (min_cost, base_acc),
                    #     xytext=(min_cost, base_acc),
                    #     xy=(min_cost, df_ac["acc"].max()),
                ...
                plot.set_title(title, fontsize=FONTSIZE_TITLE_SC)
                if ax_idx == 1:
                    plot.set_xlabel("Inference cost [%]", fontsize=FONTSIZE_LABELS_SC)
                plot.set_ylabel("Accuracy", fontsize=FONTSIZE_LABELS_SC)
                plot.tick_params(axis="both", which="major", labelsize=FONTSIZE_TICKS_SC)
                if ax_idx != 1:
            ...
            if "x5" in setting:
                ...
                raise ValueError()
            fig.savefig(
                OUTPUT_DIR_PLOTS / f"teaser_x{n_tasks}_{batch_idx}_vertical.pdf"
            )
            # fig.savefig(
            #     OUTPUT_DIR_PLOTS / f"teaser_x{n_tasks}_{batch_idx}_vertical.png"
            # )
    # ------------------------------------
    ...
    for batch_idx, method_batch in enumerate(PLOT_METHODS):
        for combined_plot_settings, combined_plot_titles in zip(
            ...
            plt.clf()
            plt.close("all")
            fig, axes = plt.subplots(nrows=1, ncols=len(combined_plot_settings),  figsize=(12, 5))
            ...
            if "x5" in setting:
                ...
                raise ValueError()
            fig.savefig(
                OUTPUT_DIR_PLOTS / f"teaser_x{n_tasks}_{batch_idx}_horizontal.pdf"
            )
            # fig.savefig(
            #     OUTPUT_DIR_PLOTS / f"teaser_x{n_tasks}_{batch_idx}_horizontal.png"
            # )
    # ------------------------------------
