...
SETTINGS = ["CIFAR100x6", "CIFAR100x11"]


PLOT_SETTINGS = [
    ["CIFAR100x6", "CIFAR100x11"],
]
PLOT_TITLES = [
    ["CIFAR100x6 | ResNet32", "CIFAR100x11 | ResNet32"],
]
...
if __name__ == "__main__":
    ...
    for batch_idx, method_batch in enumerate(PLOT_METHODS):
        for combined_plot_settings, combined_plot_titles in zip(
            ...
            plt.tight_layout()
            fig.savefig(OUTPUT_DIR_PLOTS / f"warm_start_{batch_idx}.pdf")
            # fig.savefig(OUTPUT_DIR_PLOTS / f"warm_start_{batch_idx}.png")
    ...
    combined_df.to_csv(OUTPUT_DIR_TABLES / "combined_warm_start.csv", index=False)
