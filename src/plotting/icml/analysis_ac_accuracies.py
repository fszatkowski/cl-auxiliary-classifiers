            ...
            for idx in range(len(methods)):
                ...
                for seed in seeds:
                    for task_id in task_ids:
                        task_cls_accs = [
                            df_tmp[(df_tmp["seed"] == seed) & (df_tmp["task_id"] == task_id) & (df_tmp["ic_idx"] == cls_idx)]["acc"].values[0]
                            for cls_idx in cls_ids
                        ]
                        acc_difs = [cls_acc - task_cls_accs[-1] for cls_acc in task_cls_accs]
                        relative_acc_diffs = [100 * acc_dif / task_cls_accs[-1] for acc_dif in acc_difs]

                        for ic_idx, ac_diff, ac_rel_diff in zip(cls_ids, acc_difs, relative_acc_diffs):
                ...
                colors = list(reversed(
                    sns.color_palette("tab10", n_colors=n_tasks)
                ))
                color_palette = {i: color for i, color in enumerate(colors)}
                ...
                if idx == 0:
                    method_plot.set_ylabel("Accuracy difference", fontsize=FONTSIZE_LABELS)
                else:
                ...
                if idx == 3:
                    ...
                    plt.legend(plot_handles, plot_labels, title="Task ID", fontsize=FONTSIZE_LEGEND-3, loc='upper right', ncol=5)
