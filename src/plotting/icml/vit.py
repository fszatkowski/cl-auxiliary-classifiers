...
SETTINGS = ["ImageNet100x5_vit", "ImageNet100x10_vit"]

BASELINE_NAME = "Base"
AC_NAME = "+AC"

PLOT_SETTINGS = [
    ["ImageNet100x5_vit", "ImageNet100x10_vit"],
]
PLOT_TITLES = [
    ["ImageNet100x5 | ViT-base", "ImageNet100x10 | ViT-base"],
]
...
if __name__ == "__main__":
    ...
    for batch_idx, method_batch in enumerate(PLOT_METHODS):
        for combined_plot_settings, combined_plot_titles in zip(
            ...
            plt.clf()
            plt.close("all")
            fig, axes = plt.subplots(1, len(combined_plot_settings), figsize=FIGSIZE_APPENDIX_JOINT_PLOTS)
            ...
            for ax_idx, (setting, title) in enumerate(
                ...
                if batch_idx != 0:
                    # Set lower bound on y to 10
                    plot.set_ylim(bottom=8)
                step_size = 10
            ...
            if "x5" in setting:
                ...
                raise ValueError()
            fig.savefig(OUTPUT_DIR_PLOTS / f"vit_{batch_idx}.pdf")
    ...
    combined_df.to_csv(OUTPUT_DIR_TABLES / "combined_vit.csv", index=False)
