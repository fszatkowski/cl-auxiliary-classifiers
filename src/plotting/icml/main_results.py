...
if __name__ == "__main__":
    ...
    for batch_idx, method_batch in enumerate(PLOT_METHODS):
        for combined_plot_settings, combined_plot_titles in zip(
            ...
            plt.clf()
            plt.close("all")
            fig, axes = plt.subplots(1, len(combined_plot_settings), figsize=FIGSIZE_MAIN_PAPER)
            ...
            for ax_idx, (setting, title) in enumerate(
                ...
                if ax_idx == len(combined_plot_settings) - 1:
                    ...
                    plot.legend(
                        handles=handles,
                        labels=labels,
                        fontsize=FONTSIZE_LEGEND - 2 if batch_idx == 0 else FONTSIZE_LEGEND - 4 ,
            ...
            if "x5" in setting:
                ...
                raise ValueError()
            fig.savefig(OUTPUT_DIR_PLOTS / f"main_x{n_tasks}_{batch_idx}.pdf")
