...
if __name__ == "__main__":
    ...
    def setup_fn(x):
        if isinstance(x, str):
            ...
            if 'const' in x["exp_name"]:
                setup = "Const"
            elif 'eq' in x["exp_name"]:
                setup = "Eq"
            else:
                setup = "SDN"
        else:
    ...
    for batch_idx, method_batch in enumerate(PLOT_METHODS):
        for combined_plot_settings, combined_plot_titles in zip(
            ...
            if "x5" in setting:
                ...
                raise ValueError()
            fig.savefig(OUTPUT_DIR_PLOTS / f"training_{batch_idx}.pdf")
            # fig.savefig(OUTPUT_DIR_PLOTS / f"training_{batch_idx}.png")
