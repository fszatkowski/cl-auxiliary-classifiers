...
SETTINGS = ["CIFAR100x20", "CIFAR100x50"]


PLOT_SETTINGS = [
    ["CIFAR100x20", "CIFAR100x50"],
]
PLOT_TITLES = [
    ["CIFAR100x20 | ResNet32", "CIFAR100x50 | ResNet32"],
]
...
if __name__ == "__main__":
    ...
    for batch_idx, method_batch in enumerate(PLOT_METHODS):
        for combined_plot_settings, combined_plot_titles in zip(
            ...
            plt.tight_layout()
            fig.savefig(OUTPUT_DIR_PLOTS / f"longer_tasks_{batch_idx}.pdf")
            # fig.savefig(OUTPUT_DIR_PLOTS / f"longer_tasks_{batch_idx}.png")
    ...
    combined_df.to_csv(OUTPUT_DIR_TABLES / "combined_longer_tasks.csv", index=False)
