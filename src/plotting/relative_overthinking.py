...
def get_overthinking_data(path: Path, device: str):
    ...
    for task_id in range(len(task_dirs)):
        ...
        for ac_idx, cls_hits in enumerate(hits_tag_overthinking):
            ...
            per_ac_overthinking_perc = per_ac_overthinking_hits / int(
                total_overthought_samples
            )
            outputs.append(
                {
                    "setting": setting,
...
def plot_overthinking_between_tasks(
    ...
    artifact_path = output_dir.parent.joinpath("artifacts").joinpath(
        "relative_overthinking_data.csv"
    )
    ...
    for setup in ["LP", "AC"]:
        for setting in ["CIFAR100x5", "CIFAR100x10"]:
            ...
            plot.get_figure().savefig(
                output_dir.joinpath(f"rel_overthinking_{setting}_{setup}.pdf")
            )
