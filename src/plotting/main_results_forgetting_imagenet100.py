...
SETTINGS = ["ImageNet100x5_rn18", "ImageNet100x10_rn18"]


PLOT_SETTINGS = [
    ["ImageNet100x5_rn18", "ImageNet100x10_rn18"],
]
PLOT_TITLES = [
    ["ImageNet100x5 | ResNet32", "ImageNet100x10 | ResNet32"],
]
...
if __name__ == "__main__":
    ...
    averaged_scores = load_averaged_scores(
        ROOT_DIR / "results_recomputed_in",
    ...
    plt.tight_layout()
    fig.savefig(OUTPUT_DIR_PLOTS / f"combined_in100.pdf")
    fig.savefig(OUTPUT_DIR_PLOTS / f"combined_in100.png")
    ...
    try:
        ...
        fig, axes = plt.subplots(
            len(SETTINGS),
            len(ac_indices),
            figsize=(5 * len(ac_indices), 4 * len(SETTINGS)),
        )
        for setting_idx, setting in enumerate(SETTINGS):
            for ac_enumerator, ac_idx in enumerate(ac_indices):
                ax_idx = ac_enumerator
                fig_df = ft_df[
                    (ft_df["setting"] == setting) & (ft_df["ac_idx"] == ac_idx)
                ]
                plot = sns.lineplot(
                    data=fig_df,
                    x="current_task_idx",
                ...
                plot.get_legend().remove()
        plt.tight_layout()
        fig.savefig(OUTPUT_DIR_PLOTS / f"per_ac_relative_forgetting_FT_in100.png")
    except Exception as e:
        pass
    ...
    final_df.to_csv(OUTPUT_DIR_TABLES / "combined_forgetting_in100.csv", index=False)

    md_string = final_df.to_markdown(index=False)
    md_string = md_string.replace("\\tiny{", "").replace("}", "")
    md_string = md_string.replace(" ", "")
    md_output_path = OUTPUT_DIR_TABLES / "combined_forgetting_in100.md"
