...
def get_overthinking_data(path: Path, device: str):
    ...
    for task_id in range(len(task_dirs)):
        ...
        for batch in batches_paths:
            ...
            unique_overthinking = overthought_samples[
                overthought_samples.sum(1) == 1
            ].sum(0)
    ...
    for ac_idx in unique_acs:
        ...
        ac_avg_unique_overthinking_hits = sum(
            [o["unique_overthinking_hits"] for o in outputs_ac]
        )
        ac_avg_unique_overthinking_abs = ac_avg_unique_overthinking_hits / total_samples
        ac_avg_unique_overthinking_rel = (
            ac_avg_unique_overthinking_abs / avg_overthinking
        )
...
def plot_overthinking_between_tasks(
    ...
    artifact_path = output_dir.parent.joinpath("artifacts").joinpath(
        "unique_overthinking_data.csv"
    )
    ...
    for setup in ["LP", "AC"]:
        for setting in ["CIFAR100x5", "CIFAR100x10"]:
            ...
            tmp_df = tmp_df.sort_values(
                by=["cl_method"],
                key=lambda x: x.map(
                    {"Joint": 0, "FT": 1, "FT+Ex": 2, "LwF": 3, "BiC": 4}
                ),
                ascending=True,
            )
            ...
            plot.set_xticklabels(
                ["AC1", "AC2", "AC3", "AC4", "AC5", "AC6"], fontsize=FONTSIZE_TICKS
            )
            plot.set_yticklabels(plot.get_yticklabels(), fontsize=FONTSIZE_TICKS)
            # Make legend without title
            # handles, labels = plot.get_legend_handles_labels()
            ...
            plot.get_figure().savefig(
                output_dir.joinpath(f"unique_overthinking_{setting}_{setup}.pdf")
            )
            # plot.get_figure().savefig(output_dir.joinpath(f"unique_overthinking_{setting}_{setup}.png"))
...
if __name__ == "__main__":
    ...
    result_paths = sorted(list(result_dirs.glob("CIFAR100x5/*_sdn*/*/*"))) + sorted(
